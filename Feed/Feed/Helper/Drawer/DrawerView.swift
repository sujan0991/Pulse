...
class DrawerView: UIView, drawerProtocolNew, UITableViewDelegate, UITableViewDataSource {
    ...
    convenience init(aryControllers: NSArray, isBlurEffect:Bool, isHeaderInTop:Bool, controller:UIViewController) {
        ...
        self.tblVw.register(UINib.init(nibName: "DrawerCell", bundle: bundle), forCellReuseIdentifier: "DrawerCell")
    ...
    func initialise(controllers:NSArray, isBlurEffect:Bool, isHeaderInTop:Bool, controller:UIViewController) {
        ...
        imgBG.image = imgBg ?? #imageLiteral(resourceName: "Dan-Leonard")
    ...
    func allocateLayout(controllers:NSArray, isHeaderInTop:Bool) {
        ...
        if isHeaderInTop {
            ...
            vwForHeader = UIView(frame:CGRect(x:0, y:(UIApplication.shared.keyWindow?.safeAreaInsets.top)!, width:drawerView.frame.size.width, height:75))
//            self.lblunderLine = UILabel(frame:CGRect(x:vwForHeader.frame.origin.x+10, y:vwForHeader.frame.size.height - 1 , width:vwForHeader.frame.size.width-20, height:1.0))
            tblVw.frame = CGRect(x:10, y:vwForHeader.frame.origin.y+vwForHeader.frame.size.height, width:screenSize.width/2+75, height:screenSize.height-100)

        }else{
            tblVw.frame = CGRect(x:0, y:40, width:screenSize.width/2+75, height:screenSize.height-100)
        ...
        btnLogOut = UIButton(frame:CGRect(x:90, y:14, width:vwForHeader.frame.size.width/2+30, height:25))
      //  btnLogOut.setTitle("LOG OUT", for: .normal)
        ...
        imgProPic = UIImageView(frame:CGRect(x:20, y:btnLogOut.frame.origin.y-5, width:60, height:60))
        imgProPic.image = imgBg ?? #imageLiteral(resourceName: "Dan-Leonard")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        cell.lblController.font = fontNew ?? UIFont(name: "System-reguler", size: 16)
...
//        self.delegate?.pushTo(viewController: controllerName)
    ...
    func clickOnName() {
        ...
        actDissmiss()
//        let storyBoard = UIStoryboard(name:"Main", bundle:nil)
//        let controllerName = (storyBoard.instantiateViewController(withIdentifier: "History"))
//        controllerName.hidesBottomBarWhenPushed = true
//        self.delegate?.pushTo(viewController: controllerName)
